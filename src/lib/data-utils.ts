...
export async function getAdjacentPosts(currentId: string): Promise<{
  ...
  if (isSubpost(currentId)) {
    ...
    const subposts = posts
      .filter((post) => isSubpost(post.id) && getParentId(post.id) === parentId && !post.data.draft)
      .sort((a, b) => {
...
export async function getPostsByAuthor(authorId: string): Promise<CollectionEntry<'blog'>[]> {
  const posts = await getAllPosts()
...
export function isSubpost(postId: string): boolean {
  return /^\d{4}\/\d{2}\/[^/]+$/.test(postId) ? false : postId.includes('/')
}
